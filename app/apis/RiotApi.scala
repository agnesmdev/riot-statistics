...
class RiotApiImpl @Inject()(apiClient: RiotApiClient, system: ActorSystem, configuration: Configuration)
  ...
  override def getSummoner(name: String)(region: Region): Future[Summoner] = {
    ...
    val action = () => apiClient.getSummoner(name)(region.host).get().flatMap {
      ...
      case response if response.status == Status.NOT_FOUND => throw NotFoundException(s"Summoner $name")
      case response if response.status == Status.TOO_MANY_REQUESTS => throw TooManyRequestsException(response.body)
  ...
  override def getMatch(matchId: Long)(region: Region): Future[Match] = {
    ...
    val action = () => apiClient.getMatch(matchId)(region.host).get().flatMap {
      case response if response.status == Status.OK =>
      ...
      case response if response.status == Status.NOT_FOUND => throw NotFoundException(s"Match $matchId")
      case response if response.status == Status.TOO_MANY_REQUESTS => throw TooManyRequestsException(response.body)
      case response => throw ApiException(response.status, response.body)
    }
  ...
  private def getMatches(accountId: String, region: Region, beginIndex: Int = 0, result: Seq[MatchInfo] = Nil): Future[Seq[MatchInfo]] = {
    val action = () => apiClient.getSummonerMatches(accountId, beginIndex)(region.host).get().flatMap {
      case response if response.status == Status.OK =>
      ...
      case response if response.status == Status.NOT_FOUND => Future.successful(result)
      case response if response.status == Status.TOO_MANY_REQUESTS => throw TooManyRequestsException(response.body)
      case response => throw ApiException(response.status, response.body)
    }
